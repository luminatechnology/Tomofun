    ...
    public class LUMAmazon_DEPaymentUploadProcess : PXGraph<LUMAmazon_DEPaymentUploadProcess>, PXImportAttribute.IPXPrepareItems
    {
        ...
        protected virtual IEnumerable filter()
        {
            ...
            if (filter != null)
            {
                ...
                var cmd = PaymentTransactions.View.BqlSelect.AggregateNew<Aggregate<Sum<LUMAmazonDEPaymentReport.api_total>>>();// add aggregation to the existing BQL
                foreach (LUMAmazonDEPaymentReport row in
        ...
        #region Method
        public static void GoProcessing(List<LUMAmazonDEPaymentReport> list)
        {
            var baseGraph = CreateInstance<LUMAmazon_DEPaymentUploadProcess>();
        ...
        public virtual void CreatePaymentByOrder(LUMAmazon_DEPaymentUploadProcess baseGraph, List<LUMAmazonDEPaymentReport> selectedList)
        {
            foreach (var selectedItem in selectedList)
            {
                ...
                try
                {
                    ...
                    {
                        AmazonToAcumaticaCore<LUMAmazonDEPaymentReport>.CreatePayment(selectedItem, selectedItem.API_Marketplace, baseGraph);
                        sc.Complete();
                    }
                }
